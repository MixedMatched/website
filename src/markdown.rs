...
pub fn Markdown(cx: Scope<MarkdownProps>) -> Element {
    ...
    let html_output = html_output
        .replace("<h1>", "<h1 class=\"text-4xl font-bold mb-2 dark:text-white\">")
        .replace("<h2>", "<h2 class=\"text-3xl font-bold mb-2 dark:text-white\">")
        .replace("<h3>", "<h3 class=\"text-2xl font-bold mb-2 dark:text-white\">")
        .replace("<h4>", "<h4 class=\"text-xl font-bold mb-2 dark:text-white\">")
        .replace("<h5>", "<h5 class=\"text-lg font-bold mb-2 dark:text-white\">")
        .replace("<h6>", "<h6 class=\"text-base font-bold mb-2 dark:text-white\">");

    // add tailwind to code blocks
    let html_output = html_output
        .replace("<pre>", "<pre class=\"mb-4 bg-gray-200 dark:text-white dark:bg-gray-900\">");
